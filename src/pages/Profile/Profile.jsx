...
const Profile = (props) => {
  ...
  useEffect(() => {
    const getDataGame = async () => {
      ...
      const { nick, pfpUrl, phrase } = await docSnap.data();
      setNick(nick);
      setPfpUrl(pfpUrl);
  ...
  useEffect(() => {
    if (!boolCrop) {
      //Se pone esto en falso después de croppear
      if (imgFile) {
        //SI hay archivo recibido... actualizar la imagen de usuario para subirla
        setPfpUrl(imgFile);
      }
    }
  }, [boolCrop]);
  ...
  const updatePageProfile = async (e) => {
    ...
    if (pathPic) {
      await getDownloadURL(ref(storage, pathPic)).then((url) => {
        urlPfpToUpload = url;
